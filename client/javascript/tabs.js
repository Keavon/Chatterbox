cb.tabs = function() {
	...
	addTab = function(network, channel) {
	...
	tabClickEvents = function(event) {
		...
		if (event.target.nodeName === "svg") {
			closeTab(cb.util.indexInParent(event.target.parentElement));
			return;
		} else if (event.target.parentElement.nodeName === "svg") {
			closeTab(cb.util.indexInParent(event.target.parentElement.parentElement));
			return;
		}
		
		// Check if click is on a tab
		if (event.target.nodeName.toLowerCase() === "a") {
			changeTab(cb.util.indexInParent(event.target));
			return;
		} else if (event.target.parentElement.nodeName.toLowerCase() === "a") {
			changeTab(cb.util.indexInParent(event.target.parentElement));
			return;
		} else if (event.target.parentElement.parentElement.nodeName.toLowerCase() === "a") {
			changeTab(cb.util.indexInParent(event.target.parentElement.parentElement));
			return;
		}
	};
